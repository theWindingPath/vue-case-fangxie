<template>
    ...
    <div class="goods">
        ...
        <div class="menu-wrapper" v-el:menu-dom>
            ...
            <ul>
                <li v-for="shangpin in shangpin_shuju" class="menu-item" :class="{'current': xiabiao_dangqian === $index}" @click="xuanze_zuoce_hanshu($index, $event)">
                    ...
                    <ul>
                        <li v-for="food_meige in shangpin.foods" class="food-item border-1px-suofang">
                            ...
                            <div class="content">
                                ...
                                <div class="price">
                                    <span class="newPrice">￥{{food_meige.price}}/ {{food_meige.shuliangGoumai}}个</span><span class="oldPrice" v-show="food_meige.oldPrice">￥{{food_meige.oldPrice}}</span>
                                </div>
                                <div class="cartControl-wrapperr">
                                    <v-kongjian :food_meige="food_meige"></v-kongjian>
                                </div>
                            </div>
                        </li>
                    </ul>
        ...
        <!-- 购物车组件 -->
        <v-gouwuche v-ref:gouwuche :xuanzhe_shangpin_props="xuanzhe_shangpin_props" :delivery-price="maijia_shuju.deliveryPrice" :min-price="maijia_shuju.minPrice"></v-gouwuche>
    </div>
</template>

<script type="text/ecmascript-6">
  import Gundong_chajian from 'better-scroll'; // 引入滚动第三方插件better-scroll
  import gouwuche_zujian from 'components/gouwuche_zujian_mulu/gouwuche_zujian'; // 引入购物车组件
  import kongjian_zujian from 'components/kongjian_gouwuche_zujian_mulu/kongjian_gouwuche_zujian.vue'; // 引入购物车控件
  ...
  export default {
    ...
    computed: {
        ...
        xuanzhe_shangpin_props() { // 遍历shangpin_shuju[]，取得里面的food[]里的单个food{},放到一个新的数组里返回，传到子组件gouwuche.vue里
          let shiwu_dange_shuzu = []; // 定义单个食物数组，初始为空
          this.shangpin_shuju.forEach((shangpin_dange) => { // 遍历goods获得里面单个元素
            shangpin_dange.foods.forEach((food_dange) => { // 遍历foods里的每个元素，取得每个food对象{}
            //   console.log(food_dange.shuliangGoumai);
            //   shiwu_dange_shuzu.push(food_dange); // 将获得的单个food{}放到一个数组里返回（有count的）
                if (food_dange.shuliangGoumai) { // 点击后有count的添加到数组里
                    shiwu_dange_shuzu.push(food_dange); // 将获得的单个food{}放到一个数组里返回（有count的）
                    // console.log(shiwu_dange_shuzu);
                    // return shiwu_dange_shuzu;
                }
            });
          });
          return shiwu_dange_shuzu;
    ...
    methods: {
        _chushihua_hanshu() {
            ...
            this.menu_gundong = new Gundong_chajian(this.$els.menuDom, {
                click: true // 允许点击
            }); // 实例化better-scroll
        ...
        xuanze_zuoce_hanshu(index, event) {
            if (!event._constructed) {
                return; // 浏览器原生点击事件不执行下面代码
            }
            // console.log(index);
            let zuoce_liebiao_dom = this.$els.foodsDom.getElementsByClassName('food-list-hook'); // 获取左侧dom列表
            let yuansu_dianji_dom = zuoce_liebiao_dom[index]; // 获取当前点击的dom节点
            this.foods_gundong.scrollToElement(yuansu_dianji_dom, 500); // 调用滚动函数
        },
        _xialuo_methods(target) { // 调用子组件的函数，将dom节点传过去
        //   console.log(target);
          this.$refs.gouwuche.xialuo_methods_shopcart(target);
        }
    },
    components: {
        'v-gouwuche': gouwuche_zujian, // 购物车组件注册
        'v-kongjian': kongjian_zujian
    },
    events: { // 接收子组件cartcontrol派发事件
      'zengjia_$dispatch'(target) {
        //   调用内部函数，
        this._xialuo_methods(target);
      }
    }
  };
</script>
